...
export class BillOfExchangeComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    await this.filteranytablepagination.LoadTableExport({}, { skip: 0, limit: 10 }, 'billofexchanges',this.FILTER_VALUE_LIST_NEW)?.billofexchanges().then((res) => {
  ...
  async onSubmit(value: any) {
    ...
    if (value?.value?.todate != '' && value?.value?.todate != undefined) {
      form_value = {
        buyerName: value?.value?.buyerName,
        pi_poNo: value?.value?.NO,
        billOfExchangeDate: { $gte: value?.value?.todate }
      };
      if ((value?.value?.todate != '' && value?.value?.todate != undefined) && (value?.value?.fromdate != '' && value?.value?.fromdate != undefined)) {
        form_value = {
          buyerName: value?.value?.buyerName,
          pi_poNo: value?.value?.NO,
          billOfExchangeDate: { $gte: value?.value?.todate, $lt: value?.value?.fromdate }
        };
      }
      ...
      form_value = {
        buyerName: value?.value?.buyerName,
        pi_poNo: value?.value?.NO,
        billOfExchangeDate: { $lt: value?.value?.fromdate }
      };
      if ((value?.value?.todate != '' && value?.value?.todate != undefined) && (value?.value?.fromdate != '' && value?.value?.fromdate != undefined)) {
        form_value = {
          buyerName: value?.value?.buyerName,
    ...
    if (Object.keys(removeEmptyValues(form_value))?.length != 0) {
      ...
      await this.filteranytablepagination.LoadTableExport(this.FILTER_FORM_VALUE, { skip: 0, limit: 10 }, 'billofexchanges',this.FILTER_VALUE_LIST_NEW)?.billofexchanges().then((res) => {
        this.FILTER_VALUE_LIST_NEW = res;
      });
    } else {
  ...
  exportToExcel() {
    const ws: xlsx.WorkSheet = xlsx.utils.json_to_sheet(new BillOfExchangeFormat(this.filteranytablepagination?.TABLE_CONTROLLER_DATA).get());

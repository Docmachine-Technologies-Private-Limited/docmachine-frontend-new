...
export class InwardRemittanceDisposalComponent implements OnInit {
  ...
  async ngOnInit() {
    this.actRoute.paramMap.subscribe(async (params) => {
      ...
      await this.filteranytablepagination.LoadTable(this.FILTER_FORM_VALUE, { skip: 0, limit: 10 }, 'Inward_remittance', this.FILTER_VALUE_LIST_NEW, this.GET_FILE_NAME_ROUTE)?.Inward_remittance().then((res) => {

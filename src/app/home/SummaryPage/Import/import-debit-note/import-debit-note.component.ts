...
export class ImportDebitNoteComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    await this.filteranytablepagination.LoadTableImport({}, { skip: 0, limit: 10 }, 'debitnotes',this.FILTER_VALUE_LIST_NEW)?.debitnotes().then((res) => {
 ...
 async onSubmit(value: any) {
    ...
    if (Object.keys(removeEmptyValues(form_value))?.length != 0) {
      ...
      await this.filteranytablepagination.LoadTableImport(this.FILTER_FORM_VALUE, { skip: 0, limit: 10 }, 'debitnotes',this.FILTER_VALUE_LIST_NEW)?.debitnotes().then((res) => {
        this.FILTER_VALUE_LIST_NEW = res;
      });
    } else {

...
export class BOEORMMactchData {
    ...
    getShippingBill(buyerName: any, type: any) {
        ...
        if (buyerName != undefined && buyerName != null && buyerName != '') {
            this.documentService.filterAnyTable({benneName:buyerName?.benneName},'boerecords').subscribe((res: any) => {
                let data: any = [];
                this.SHIPPING_BILL_DATA = []
                this.SHIPPING_BILL_DATA_LINKAGE = [];
                this.SELECTED_COMMERICAIL_DATA = []
                ...
                res?.data?.forEach(element => {
                    ...
                    element['ORM_ADVICE_SUM'] = '0'
                    element['ORM_ADVICE_DATA'] = []
                    ...
                    if (type == "MatchOff") {
                        element?.CI_REF?.forEach((element1: any, index) => {
                ...
                if (this.IS_AGAINST_ADVANCE_YES_NO == true) {
                    this.TRANSACTION_SHIPPING_BILL = this.SHIPPING_BILL_DATA?.filter((item: any) => item?.ORMdetails?.length != 0)
    ...
    getBuyerList() {
        this.userService.getBene(1).subscribe((res: any) => {console.log(res?.data,"getBene");this.BUYER_LIST = res?.data});
    }
    ...
    setSelectedShippingBill($event, data: any) {
        if ($event?.target?.checked == true) {
            ...
            this.SHIPPING_BILL_DATA?.forEach(element => {
                ...
                element?.CI_REF?.forEach(commercialdetailselement => {
                    ...
                    commercialdetailselement['ORM_ADVICE_SUM'] = '0'
                    commercialdetailselement['ORM_ADVICE_DATA'] = []
            ...
            if (parseFloat(this.SELECTED_COMMERICAIL_DATA?.amount) == parseFloat(sum_of_irm)) {
                ...
                this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_SUM'] = sum_of_irm
                this.SELECTED_COMMERICAIL_DATA['debitAmount'] = '0';
            } else {
                this.SELECTED_COMMERICAIL_DATA['CheckBoxEnabled'] = false;
                this.SELECTED_COMMERICAIL_DATA['Firxbutton'] = false;
                this.SELECTED_COMMERICAIL_DATA['SB_Amout_Realized'] = '0';
                this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_SUM'] = '0'
                this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_DATA'] = []
                this.SELECTED_COMMERICAIL_DATA['debitAmount'] = '0';
    ...
    FirxSelection($event: any, index: any, data: any) {
        if ($event?.target?.checked == true) {
          console.log(this.SELECTED_SHIPPING_BILL,"SELECTED_SHIPPING_BILL")
            if (this.SELECTED_SHIPPING_BILL?.length != 0) {
                if (this.SELECTED_COMMERICAIL_DATA?.length != 0) {
                    this.SELECTED_SHIPPING_BILL['ORM_ADVICE_DATA']?.push(data);
                    this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_DATA']?.push(data);
                    data.Enabled = true;
                    if (parseFloat(this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_SUM']) < parseFloat(this.SELECTED_COMMERICAIL_DATA?.amount)) {
                        let amount: any = 0;
                        ...
                        this.SELECTED_SHIPPING_BILL['ORM_ADVICE_DATA']?.pop();
                        this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_DATA']?.pop();
                        $event.target.checked = false;
                        data.isChecked = false;
                        data.Enabled = false;
                        this.toastr.error("You've exceeded the maximum transaction amount set by your BOE amount..");
                    }
                    let TOTAL_CI_FIRX_SELECTED_AMOUNT = this.SELECTED_COMMERICAIL_DATA?.ORM_ADVICE_DATA?.reduce((a, b) => parseFloat(a) + parseFloat(b?.InputValue), 0);
                    this.SELECTED_SHIPPING_BILL['ORM_ADVICE_SUM'] = TOTAL_CI_FIRX_SELECTED_AMOUNT;
                    this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_SUM'] = TOTAL_CI_FIRX_SELECTED_AMOUNT;
                } else {
                    $event.target.checked = false
                ...
                $event.target.checked = false
                this.toastr.error("Please select atleast one BOE bill.")
            }
        } else {
            let findIndex: any = this.SELECTED_SHIPPING_BILL?.ORM_ADVICE_DATA?.findIndex(item => item?._id == data?._id);
            this.SELECTED_SHIPPING_BILL?.ORM_ADVICE_DATA?.splice(findIndex, 1);
            this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_DATA']?.splice(findIndex, 1);
            let TOTAL_CI_FIRX_SELECTED_AMOUNT = this.SELECTED_COMMERICAIL_DATA?.ORM_ADVICE_DATA?.reduce((a, b) => parseFloat(a) + parseFloat(b?.InputValue), 0);
            this.SELECTED_SHIPPING_BILL['ORM_ADVICE_SUM'] = TOTAL_CI_FIRX_SELECTED_AMOUNT;
            this.SELECTED_COMMERICAIL_DATA['ORM_ADVICE_SUM'] = TOTAL_CI_FIRX_SELECTED_AMOUNT;
    ...
    SaveChanges(data: any, index: any) {
        if (data?.ORM_ADVICE_DATA?.length != 0 && data?.ORM_ADVICE_DATA != undefined) {
            this.tp = {
                firxNumber: [],
                firxDate: [],
                firxCurrency: [],
            ...
            data?.ORM_ADVICE_DATA?.forEach(element => {
            ...
            data?.CI_REF?.forEach(async (element) => {
                if (element?.ORM_ADVICE_DATA?.length != 0 && element?.ORM_ADVICE_DATA != undefined) {
                    let FIRX_ID: any = [];
                    let FIRX_DATA: any = [];
                    await element?.ORM_ADVICE_DATA?.forEach(async (element1) => {
            ...
            this.documentService.Update_Amount_by_Table({
                ...
                let sumfixAmount2 = 0;
                if (parseFloat(sumfixAmount) == parseFloat(data?.balanceAmount)) {
                    sumfixAmount2 = parseFloat(data?.balanceAmount) - parseFloat(sumfixAmount)
                } else {
                    sumfixAmount2 = parseFloat(data?.balanceAmount) - (parseFloat(sumfixAmount) + parseFloat(sumfixCommisionAmount))
                }
                this.documentService.Update_Amount_by_Table({
                    tableName: 'boerecords',
                    id: data?._id,
                    query: { balanceAmount: sumfixAmount2 }
                }).subscribe((r3: any) => {
        ...
        this.confrimModel.YesNoDialogModel("Reset All Data<br/> Do you want d-select all data with this shipping bill no. : " + data?.boeNumber, "", (value: any) => {
            if (value?.value === "Yes") {
                this.documentService.Update_Amount_by_Table({
                    ...
                    query: {
                        ORMdetails: [],
                        balanceAmount: data?.invoiceAmount
                    }
                        ...
                        element?.MatchOffData?.forEach(IRM_REF_element => {
                            this.documentService.Update_Amount_by_Table({
                                ...
                                await this.documentService.Update_Amount_by_Table({
                                    tableName: 'commercials',
                                    id: element?._id,
                                    query: { ORM_Ref: [], TransctionEnabled: false, MatchOffData: [] }
                                }).subscribe((r2: any) => {

                                });
                            })
                        });
                        if ((index + 1) == data?.CI_REF?.length) {
                            this.toastr.success("Update Changes...")
                            this.getShippingBill(this.SELECTED_BUYER_NAME, "MatchOff");
                        }
                    }

...
import { ImportBillLodgementData } from './import-bill-lodgemet-data';
...
@Component({
  selector: 'app-import-bill-lodgement',
  templateUrl: './import-bill-lodgement.component.html',
  styleUrls: ['./import-bill-lodgement.component.scss', '../../../../commoncss/common.component.scss']
})
export class ImportBillLodgementComponent implements OnInit {
  @Input('data') data: any = [];
  ...
  constructor(public sanitizer: DomSanitizer,
    ...
    public ImportBillLodgementData: ImportBillLodgementData,
    ...
    ImportBillLodgementData.clear();
  }

  ngOnInit(): void {
    this.actRoute.paramMap.subscribe(async (params) => {
      this.ImportBillLodgementData.clear();
      this.PREVIWES_URL = ''
    ...
    setTimeout(() => {
      this.validator.buildForm({
        ...
        PAYMENTINSTRUCTION: {
          type: "yesnocheckbox",
          value: '',
          label: "PAYMENT INSTRUCTION (*BOE Mandatory)?",
          rules: {
            required: true,
          },
          YesNo: '',
  ...
  async getS3Url() {
    return new Promise(async (reslove, reject) => {
      ...
      await this.userService?.UploadS3Buket({
        fileName: this.guid() + '.pdf', buffer: this.PREVIWES_URL != '' ? this.PREVIWES_URL : this.ImportBillLodgementData.PREVIWES_URL,
  ...
  SendApproval(Status: string, UniqueId: any) {
    if (UniqueId != null) {
      ...
      var approval_data: any = {
        ...
        tableName: 'Import-Bill-Lodgement',
        deleteflag: '-1',
        ...
        if (res?.id != approval_data?.id || res == undefined) {
          this.AprrovalPendingRejectService.DownloadByRole_Transaction_Type(this.validator.userData['RoleCheckbox'], approval_data, () => {
            var data: any = {
              ...
              TypeTransaction: 'Import-Bill-Lodgement',
              fileType: this.validator.userData?.sideMenu,
